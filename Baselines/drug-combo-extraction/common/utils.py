...
from preprocessing.data_loader import DatasetRow
...
class ErrorAnalysisAttributes:
    ...
    def get_row(self):
        return [self.row_id, self.sentence, self.entities, self.paragraph, self.ground_truth_label, self.predicted_label, self.sentence_length, self.num_spans_in_ground_truth_relation, self.avg_span_distance_in_ground_truth_relation]
...
def write_error_analysis_file(dataset: List[Dict], test_data_raw: List[Dict], test_row_ids: List[str], test_predictions: List[int], fname: str):
    ...
    with open(fname, 'w') as out_file:
        ...
        in_sentence_correct = 0
        in_sentence_incorrect = 0
        intersentence_correct = 0
        intersentence_incorrect = 0
        without_condition_correct = 0
        without_condition_incorrect = 0
        with_condition_correct = 0
        with_condition_incorrect = 0
        ...
        for dataset_row in dataset:
            ...
            sentence = full_document['sentence']
            sentences = Paragraph(sentence).sentences
            sentence_bd = []
            for sen in sentences:
                sentence_bd.append((sen.start, sen.end))
            is_cross_sentence = False
            rel = dataset_row["drug_indices"]
            entities =  full_document["spans"]
            rel_min = 99999999
            rel_max = 0
            for entity in entities:
                if entity['span_id'] in rel:
                    rel_min = min(rel_min, entity['start'])
                    rel_max = max(rel_max, entity['end'])
            for bd in sentence_bd:
                if rel_min > bd[0]:
                    # check if it ends
                    if rel_max > bd[1]:
                        is_cross_sentence = True
                    break
            if error_analysis_attributes.predicted_label == error_analysis_attributes.ground_truth_label:
                if error_analysis_attributes.num_spans_in_ground_truth_relation == 3:
                    if error_analysis_attributes.ground_truth_label == 1:
                        nc_tp += 1
                    else:
                        nc_tn += 1
                    without_condition_correct += 1
                else: 
                    if error_analysis_attributes.ground_truth_label == 1:
                        c_tp += 1
                    else:
                        c_tn += 1
                    with_condition_correct += 1
                if is_cross_sentence:
                    if error_analysis_attributes.ground_truth_label == 1:
                        out_tp += 1
                    else:
                        out_tn += 1
                    intersentence_correct += 1
                else:
                    if error_analysis_attributes.ground_truth_label == 1:
                        in_tp += 1
                    else:
                        in_tn += 1
                    in_sentence_correct += 1
            elif error_analysis_attributes.predicted_label != error_analysis_attributes.ground_truth_label:
                if error_analysis_attributes.num_spans_in_ground_truth_relation == 3:
                    if error_analysis_attributes.ground_truth_label == 1:
                        nc_fn += 1
                    else:
                        nc_fp += 1
                    without_condition_incorrect += 1
                else:
                    if error_analysis_attributes.ground_truth_label == 1:
                        c_fn += 1
                    else:
                        c_fp += 1
                    with_condition_incorrect += 1
                if is_cross_sentence:
                    if error_analysis_attributes.ground_truth_label == 1:
                        out_fn += 1
                    else:
                        out_fp += 1
                    intersentence_incorrect += 1
                else:
                    if error_analysis_attributes.ground_truth_label == 1:
                        in_fn += 1
                    else:
                        in_fp += 1
                    in_sentence_incorrect += 1
    
        in_precision = in_tp / (in_tp + in_fp)
        in_recall = in_tp / (in_tp + in_fn)
        in_f1 = 2 * in_precision * in_recall / (in_precision + in_recall)

        out_precision = out_tp / (out_tp + out_fp)
        ...
        nc_precision = nc_tp / (nc_tp + nc_fp)
        nc_recall = nc_tp / (nc_tp + nc_fn)
        nc_f1 = 2 * nc_precision * nc_recall / (nc_precision + nc_recall)


    print(in_precision, in_recall, in_f1)
    print(out_precision, out_recall, out_f1)
    print(c_precision, c_recall, c_f1)
    print(nc_precision, nc_recall, nc_f1)


    print('in_sentence_correct', in_sentence_correct)
    print('in_sentence_incorrect', in_sentence_incorrect)
    print('intersentence_correct', intersentence_correct)
    print('intersentence_incorrect', intersentence_incorrect)
    print('without_condition_correct', without_condition_correct)
    print('without_condition_incorrect', without_condition_incorrect)
    print('with_condition_correct', with_condition_correct)
    print('with_condition_incorrect',with_condition_incorrect)
